...
[![Hugging Face – sana1.5_siglipzoom](https://img.shields.io/badge/HuggingFace-sana1.5_siglipzoom-ffcc4d?style=for-the-badge&logo=huggingface&logoColor=black)](https://huggingface.co/AmitIsraeli/sana1.5_siglipzoom_glb_step30000)  

This experiment turns **Sana 1.5** into an **img2img model with a continuous zoom controller**, driven by **SigLIP** image embeddings.

At a high level:

- Take a collection of **GLB objects** and render them offline at multiple zoom levels (e.g. `zoom_0`, `zoom_1`, …, `zoom_N`) to create a 2D image dataset.
- Encode the **zero‑zoom render** (`zoom_0`) with **SigLIP**, then pass that embedding through a small MLP to get an **object token**.
- Map the scalar zoom value `z ∈ [0, 1]` through a separate MLP to obtain a **zoom token**.
- Append both tokens (object + zoom) to the already cached **text encoding** (sometimes an object‑specific description, sometimes a generic class prompt), and fine‑tune **only the Sana 1.5 transformer** with **LoRA**.
- At inference, given a **zero‑zoom reference image**, a **target zoom value** (and optional text description), the model generates a **single image at the requested zoom**, without re‑rendering the GLB.
...
2. **SigLIP object token**  
   - Take the zero‑zoom image (`zoom_0`), encode it with SigLIP, and feed the embedding into a small MLP to produce an **object token**.
3. **Zoom token (scalar → embedding)**  
   - Normalize the desired zoom value `z ∈ [0, 1]` and pass it through a separate MLP to obtain a **zoom token**.
4. **Text + tokens conditioning**  
   - Run the text encoder on an object‑level or class‑level prompt, then append the **object token** and **zoom token** as two additional tokens to the cached text encoding.
5. **Img2img Sana 1.5**
   - The LoRA‑tuned transformer learns to keep the object appearance consistent while changing only the zoom level.


### Reference & Zoom Sweep Example

| Reference zero‑zoom view | Zoom controller video                                                            |
|--------------------------|----------------------------------------------------------------------------------|
...
The table above shows a **single reference render** (left) and a **zoom sweep** generated by the zoom‑controlled Sana 1.5 model (right), where only the zoom value is changed while the object identity and style remain consistent.

## Sana-Video Tom & Jerry Experiment

[![Project Site – Tom & Jerry Research Log](https://img.shields.io/badge/Project_Site-Tom_%26_Jerry_Research_Log-7b5cff?style=for-the-badge&logo=google-chrome&logoColor=white)](https://amit154154.github.io/Sana-Simplified/)
